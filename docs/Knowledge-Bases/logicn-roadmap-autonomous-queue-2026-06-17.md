...
- **Classification 🟡/🔴 (harness-gated)** — extending the sync tier to all nodes touches execution fidelity, so it admits only behind the 0014 differential proving sync ≡ walker for the newly-covered nodes (variant G already shows decision-identity). NOT 🟢. Suspend only at the real effect boundary via the capability handler (LogicN's colorless/`Io`-parameter analogue; `async`/`await` stay reserved, never a colored language feature).
- Refs: KB [[logicn-tree-walker-speed-and-photonic-governance]] §1-§2; R&D bridge task **0027** (decouple-governance-from-decolored-eval). Companion measured win to queue: flat SoA `Int32Array` AST (2.2×).
